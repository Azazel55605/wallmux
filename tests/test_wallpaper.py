...
def test_set_image_executes_awww_and_saves_state(tmp_path: Path) -> None:
    ...
    result = set_wallpaper(image, "DP-1", runner=runner, state_path=state_path)
...
def test_set_video_starts_mpvpaper_and_saves_pid(tmp_path: Path) -> None:
    ...
    result = set_wallpaper(video, "eDP-1", runner=runner, state_path=state_path)
...
def test_restore_executes_saved_state(tmp_path: Path) -> None:
    ...
    image.write_bytes(b"")
    set_wallpaper(image, "DP-1", runner=first_runner, state_path=state_path)

    results = restore_wallpapers(runner=restore_runner, state_path=state_path)
...
def test_replaces_tracked_video_process(tmp_path: Path, monkeypatch) -> None:
    ...
    set_wallpaper(first_video, "DP-1", runner=runner, state_path=state_path)
    set_wallpaper(second_video, "DP-1", runner=runner, state_path=state_path)
